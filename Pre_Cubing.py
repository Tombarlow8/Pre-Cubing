...
def load_products_from_csv(file_name):
	...
	data_frame.sort_values(by=['haz_class'])
...
def Create_box_group(current_group, groups_of_products, our_products, product, temp_vol):
	...
	haz_check = check_hazards_in_group(product, current_group)

	if haz_check == False:
	    temp_vol += product.volume
	    # print(f"temp vol: {temp_vol} ")
	    if temp_vol < box4vol and len(our_products) != 1:
...
def group_products_box_groups(our_products):
    ...
    current_group = []  
    temp_vol = 0
    while len(our_products) != 0:
            temp_vol = Create_box_group(current_group, groups_of_products, our_products, our_products[0], temp_vol)
...
def pre_cubing_results():
	...
	if len(somewhere_else)  > 0:
		print("ERROR: There is an exception for the pre-cubing statements")
